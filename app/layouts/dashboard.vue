...
// Check for past_due subscription status
const activeSub = computed(() => {
  const subs = activeOrg.value?.data?.subscriptions as any[]
  if (!subs || subs.length === 0)
    return null
  return subs.find((s: any) => s.status === 'active' || s.status === 'trialing' || s.status === 'past_due') || subs[0]
})

const isPaymentFailed = computed(() => activeSub.value?.status === 'past_due')

// Billing portal for fixing payment
